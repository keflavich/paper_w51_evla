...
\subsection{New continuum sources}
...
columns.  The first shows the peak flux density within a box with size
$6\times6$ beams, which in some cases is negative because the source sits in a
region affected by large angular scale negative bowls.  The second, `Peak -
Background', shows the peak flux minus the lowest value within that same cutout
region.  When these two values agree, they are reliable, but when they differ
significantly they are probably affected by image reconstruction artifacts.
...
blueshifted lobe is evident at -50 \kms in both cubes, and neither show a redshifted
counterpart.  The H77$\alpha$ data reveal that there is not an extincted counterpart.}
...
high-mass star formation.  It will be a powerful tool for studying the earliest
...
JVLA surveys (e.g., ... et al) may detect a significant population of
proto-O-stars, though with its limited depth it may only detect source out to
distance XXX.
