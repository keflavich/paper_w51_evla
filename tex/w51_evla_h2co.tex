...
\titlerunning{JVLA observations of W51}
...
(3) There are two clearly detected protoclusters, W51e and W51 IRS2.  The
...
It is possible that the O-stars providing most of the observed reprocessed
...
% vdisp
The resulting 1D velocity dispersion is $\sigma=2.0$ \kms if we exclude the
uncertain sources e9 and e10; with e9 and e10 included the dispersion increases
to 4.2 \kms.
This velocity dispersion is confined to $r<5.4\arcsec$ or $r<0.13$ pc, though
the (proto)stars are not symmetrically distributed.
In the more compact and round e1 subcluster, where the (proto)stars are symmetrically
distributed, the velocity dispersion is still $\sigma=2.0 \kms$, but the radius
The more compact and round e1 subcluster containing e1, e3, e4, e8, and the molecular
emission regions e8mol and e10mol has a velocity dispersion $\sigma=2.0$ \kms
in  $r=2.9$ \arcsec (0.07 pc).

% cluster_properties and vdisp
If we assume all 6 \hchii regions in the e1 subcluster represent OB stars
...
mass $M_{cluster}=600\pm170$ \msun.  Assuming the stars are virialized, which
is a good assumption if the stellar mass is dominant \citep{Kruijssen2012b},
the implied stellar virial mass in e1 is $M_{vir,*}=670$ \msun assuming the
stars follow a Plummer profile \citep{Portegies-Zwart2010a}.  The lower limit
on the gas density from seeing \formaldehyde in emission is $n>10^6$ \percc,
which implies a gas mass $M_{gas} > 100$ \msun within e1.  Overall, the
observations of e1 are consistent with it being a stellar-dominated but
nonetheless gas-rich ($M_{gas}/M_{*} > 0.15$) system.

%Its implied density if it is gas-dominated is
%$n(\hh)\sim3\ee6$ \percc ($M_{vir,gas}=330$ \msun).  Such a high density is
%consistent with the observed \formaldehyde emission toward e8 and the presence
%of hypercompact \hii regions, which require densities $n\gtrsim10^6$ \percc to
%explain their emission.  However,   The stellar virial
%mass is higher than the gas virial mass, $M_{vir,*}=670$ \msun \citep[assuming
%a Plummer profile]{Portegies-Zwart2010a} and consistent with the IMF-derived
%mass.  The equivalent \hh number density is $n(\hh)=7\ee{6}$, implying a coarse
%lower-limit on the gas to star mass ratio $M_{gas}/M_{*} \gtrsim 0.15$.
...
However, W51e20 and the related W51e candidate CWBs may constitute an
...
In IRS2, accretion is ongoing despite much of its volume being evacuated of
...
has been concluded in other high-mass protoclusters \citep{Galvan-Madrid2013a}.
...
\todo{To the journal: we would like this to be hosted by CDS.}
