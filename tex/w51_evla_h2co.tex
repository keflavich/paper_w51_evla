...
\abstract
{
We present new JVLA observations of the high-mass cluster-forming region W51
Main / IRS2 from 2 to 16 GHz with resolution
...
(3) There are two clearly detected clusters, W51e and W51 IRS 2.  The
...
star formation process by consuming all of their gas (exhaustion) or
by removing it via feedback (expulsion)
\citep{Longmore2014a,Matzner2015a}.  This distinction governs whether the
...
while the achieved RMS is $\sim30-60$ \microjy.  There is a large amount of
resolved-out structure in the images that hinders cleaning.  The images are
generally artifact-dominated at the low-signal end, where inadequately cleaned
sidelobes of bright features provide significant non-gaussian noise.  Some of
these artifacts can be seen in Figure \ref{fig:coverview}.
...
{The Ku-band (14.5 GHz) image of the W51 region produced with a combination of
JVLA B and D array data using uniform weighting.  The synthesized beam size is
$\theta_{FWHM}\approx0.33\arcsec$.}
{fig:kuoverview}{0.9}{6.5in}

\Figure{figures/diffuse/W51_C_overview_pointsourcelabels.pdf}
{The C-band (4.9 GHz) image of the W51A region with point sources labeled.  The
...
\begin{enumerate}
    ...
    \item Section \ref{sec:lacyjet}: Detection of a $\sim0.1$ pc ionized
        outflow in H77$\alpha$ that had previously been observed in
...
($A_V>100$, based on mm emission).  The
...
e22, have clear NIR K-band counterparts, though the UGPS cataloged sources do not
align with the evident NIR K-band sources. We nonetheless consider these sources
...
than the true source location in the NACO and UKIDSS images.  Despite this
confusion, we regard the NIR, X-ray, and radio features to have a common
origin.
...
There are no known associated sources, though in the Spitzer GLIMPSE
\citep{Benjamin2003a} images, there is some diffuse emission at this location.
The bow-like structure points away from e11, suggesting that it is the driving
source.
...
deeper, higher-resolution data represent a dramatic imrpovement in image
quality.  Where in previous observations, a relatively smooth and clumpy
structure was observed, the new images reveal a network of wispy, filamentary
structures.  Figure \ref{fig:w51mainpeak} shows the region between the W51e and
...
somewhere between one quarter and one half the bolometric luminosity of the W51
Main/IRS2 complex.
...
region that contains no known OB stars.  The presence of multiple features
...
molecular material, to the filament.  It is possible that there are additional
...
{({\it a}) A C-band (4.9 GHz) image of the W51 Main peak intensity region.  This region
accounts for more than half of the radio luminosity of W51.  The colorbar has been selected
to emphasize the pointlike sources in the image center.  The synthesized beam size
...
Additionally, the W51 Main ridge is detected in H77$\alpha$.  Filament
...
the line of sight but are clearly distinct in velocity (Figure
...
arrow in Figure \ref{fig:w51mainpeak}), orthogonal to the direction toward
...
structure like that expected from disk-driven outflows (though this flow is
driven by a cluster of stars, not a disk).  This flow structure likely
indicates that the cluster has evacuated a large cavity approximately along the
line of sight, consistent with the low observed extinction toward
IRS2.  
...
occurs very close to the rest velocity of the He77$\alpha$ line.  If
...
regions are clearly identified as resolved sources, with $r>0.005$
pc\footnote{The
...
Radio jets are sometimes observed to have negative spectral indices, and they
are often faint at distances comparable to W51
\citep{Moscadelli2013a,Moscadelli2016a}.  However, previous radio jets have
been identified by their association with \methanol or \water masers, and the
faint continuum sources we discuss here do not have any associated masers.
Absent any signs of ongoing accretion, jets are not the most likely source of
the observed emission, but the current data cannot definitively rule them out.
...
all of the W51 Main compact sources are within a $\sim 0.5$ pc sphere.
...
None of these regions have corresponding \oneone emission in our data.  These
nondetections are likely because our brightness sensitivity at C-band (4.9 GHz)
is very poor.  The upper limit is $T_B<300$ K (Table \ref{tab:cubes}), ruling
out the presence of strong masers.  Additionally, the foreground cloud is a
much stronger absorber in the \oneone line, so any emission is likely to be
obscured by foreground absorption.
...
\citep{Mangum1993a}.  The core radius, as meaured by fitting a 2D gaussian to
...
($m_K \sim 15$, and the emission is extended) hints against this interpretation.
...
$L_{rad}<10^{28}$ erg \pers, or a flux
...
caused by variable accretion flow
\citep{Peters2010c,Galvan-Madrid2011d,dePree2014a}.  In this scenario, the
...
$r_{cluster}=1.5$ pc.  The rest of the ionized gas is bound to the system, with
thermal line width $c_s\approx7.7$ \kms.

We use Equation 14.28 of \citet{Wilson2009a} to estimate the emission measure:
\begin{equation}
    EM = 5.2\ee{-4} 
...
\end{equation}
To estimate an upper limit, we use the peak brightness in the shell at 37.5
\kms of 1.7 mJy beam$^{-1}$ in a 10 \kms band (60 K;
 see Figure
...
clump $t>6$ Myr.  The free-fall time for the cluster is $t_{ff} < 0.5$ Myr,
so the photoevaporation timescale is at least an order of magnitude longer.
Given the conservative estimates adopted above, the photoevaporative mass
...
(Section \ref{sec:irs2outflow}).  Because it contains many stars that appear to
have no ongoing accretion and it has expelled or consumed a large fraction of
its gas, the IRS2 cluster is older than W51 e1/e2.  Similarly, because they
have no evident surrounding gas or suggestions of ongoing accretion, the W51
e20 stars are likely the oldest in the W51 Main
system.
...
bow shock might generate the detected emission.  This scenario can be readily
...
(MYSOs) like IRS2E \citep{Figueredo2008a}, and the numerous \hchii regions that
are likely to be accreting, despite the fact that most radiation is clearly
escaping at least from IRS2.  The timescale for gas evacuation in IRS2 at the
presently observed mass-loss rate (averaged over $10^4$ years) is $>10\times$
longer than the gas free-fall time (Section \ref{sec:irs2outflow}).  These
points support the \citet{Matzner2015a} argument that the most massive clumps
in the galaxy  end their formation by exhausting their parent reservoir rather
than by disrupting their parent cloud via feedback.
...
nearest part of the W51 Main shell at a separation of 0.35 parsecs with a
...
Since the free-fall time is short within these clusters ($t_{ff} < 0.5$
...
is a clear deficit of molecular gas as traced by \ceighteeno \citep[Figure
