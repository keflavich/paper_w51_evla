...
The W51d CWB candidates, by contrast, are each isolated and distributed around
W51 IRS2 (Figure \ref{fig:coverview_pointsrcs}).  These could therefore be
runaway stars from the IRS 2 cluster.  d5
is the furthest of these in projection, so with a distance of 0.9 pc, its
ejection time is $t = 0.08 Myr \left(v_*/ 10 \kms\right)^{-1}$.  The runaway
scenario may also provide an alternate explanation for the observed SEDs:
synchrotron emission from the wind-ISM
...
The ongoing accretion onto the source IRS2E \citep{Figueredo2008a} and probable
ongoing accretion onto the hot cores and HCHII regions throughout these
clusters implies that ionizing radiation is, at present, not halting star
